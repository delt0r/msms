...
/**
 * CHashSet with idenitiy rather than .equals.
 * @author greg ewing
 * 
 * 
 * @param <T>
...
public class CIdentityHashSet<T> implements Set<T> {
	...
	public CIdentityHashSet(Collection<? extends T> links) {
		this(links.size(),.75);
		addAll(links);
	}
	...
	public boolean add(T e) {
		...
		int code=System.identityHashCode(e);
		int hash =hash(code);
		...
		if (e==table[hash] || e==table[hash2] || e==table[hash3])
			return false;
		if(table[hash]==null){
			table[hash]=e;
	...
	private boolean pushInsert(T e) {
		...
		while (true) {
			for (int i =0; i < size; i++) {
				...
				code=System.identityHashCode(e);
				hash =hash(code);
				hash2 = hash2(code);
				hash3 =hash3(code);
				hash =hash & mask;
	...
	public boolean addAll(CIdentityHashSet<? extends T> set){
		boolean b=false;
