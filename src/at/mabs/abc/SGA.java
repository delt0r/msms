...
/**
 * same idea as ABC. Use a annotated command line and a few parameters to add. However output is
 * much smaller. There is a Trace and just the output parameters. File and std out are the bulk
 * options. Still a lot of cut/paste from NewABC. Need to refactor.
 * 
 * @author bob
 * 
 */
...
public class SGA {
	...
	public void run() {
		...
		GradFunction better =new FastGradFunction();
		...
		for (int i =0; i < betterStarts.length; i++) {
			...
			double[] np =null;// genericKWAlgo(args, priors, betterStarts[i].point, 1, 2000,
								// simple);
			if (np == null)
				continue;
		...
		// System.out.println(preconditioned+"\t"+priors);
	...
	private double[] gradFast(String[] args, List<PriorDensity> priors, double[] x, double delta, int n) {
	...
	//	System.out.println("1:"+Arrays.toString(rv1));
		...
		double da =density(args, priors, rv1, n);
		double db =density(args, priors, rv2, n);
		double g=(Math.log(da) - Math.log(db)) / (2 * delta);
	...
	public class FastGradFunction extends GradFunction {
		public double[] grad(String[] args, List<PriorDensity> priors, double[] x, double delta, int n) {
			return gradFast(args, priors, x, delta, n);
		}
