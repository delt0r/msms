...
/**
 * this class handles all the reading from a reader and marshaling frequencys to traces.
 * 
...
public class ReaderSelection implements SelectionSimulator {
	...
	public List<ModelEvent> init(ModelHistroy modelHistory) {
		...
		//and the start time. //Here we assume a bi allele state so we have an implied deme count...
		...
		SelectionStartEvent sse=new SelectionStartEvent((long)((firstPastward.time)*genScale), firstPastward.frequencys);
		
		double[] fs=lastPastward.frequencys;
		//System.out.println("LAST:"+lastPastward.time);
		FrequencyState state=new FrequencyState(fs.length, fs.length);
		for(int i=0;i<fs.length;i++){
			state.setFrequency(i, 1, fs[i]);
			state.setFrequency(i, 0, 1-fs[i]);
		}
		SelectionEndTimeEvent see=new SelectionEndTimeEvent((long)(lastPastward.time*genScale+1),state);//FIXME the +1 thing. 
	...
	public List<ModelEvent> forwardSimulator(Model model, ModelHistroy modelHistory, SelectionStrengthModel[] ssm, SuperFrequencyTrace frequencys) {
		//note that frequencys only need to be defined between the first and last specified times.
//		FrequencyEntry lastPastward=data.get(0);
//		FrequencyEntry firstPastward=data.get(data.size()-1);
...
//			double realTime=(double)frequencys.getTime()/(4*N);
//			//System.out.println("Real:"+realTime+"\t"+frequencys.getTime()+"\t"+N);
//			double[][] data=frequencys.current();
//			putFrequency(realTime, data);
//			frequencys.previous(false);
//		}
//		double realTime=frequencys.getTime()/4*N;
//		double[][] data=frequencys.current();
//		//System.out.println("Real+:"+realTime+"\t"+frequencys.getTime()+"\t"+N);
//		putFrequency(realTime, data);
//		return new ArrayList<ModelEvent>();
	...
	private void putFrequency(double time,double[][] array){
		//so first we just find the relevent entry. forwards then backwards. 
	
		FrequencyEntry e=data.get(currentIndex);
		while(e.time>time && currentIndex<data.size()){
			currentIndex++;
			e=data.get(currentIndex);
		}
		while(e.time<time && currentIndex>0){
			currentIndex--;
			e=data.get(currentIndex);
		}
		
		FrequencyEntry next=e;
		...
		//System.out.println("Index:"+currentIndex+"\t"+e+"\t"+t+"\t"+time);
...
//		//FrequencyEntry contains a entry for each non wild type allele for each deme. hence we only can tell once we know how many demes!
//		int demeCount=array.length;
//		int alleleCount=array[0].length;
//		int index=0;
//		double[] entryData=e.frequencys;
